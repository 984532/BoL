...
function OnLoad() -- This is called once the script is loaded in game.
	...
	PrintChat("[ALPHA] 984Riven - Script Loaded, enjoy !") -- We are printing this once when the game loads. You can change "Ryze loaded!" to whatever you want.
...
function ConfigMenu()
	...
	Config = scriptConfig("984Riven - Config", "984Riven") -- We are setting "Config" as our script config. It will put a Menu in the game called "My Ryze".
...
function OnTick() -- Any function is under this function will be loaded every 10 or so ms. Which means it will be always loaded. We can add our Checks etc here.
	Checks() -- We are adding our Checks function to be loaded all the time from the start of the game to the end.
...
function OnDraw() -- Drawing function which is also called like OnTick.
	if Config.Draw.DrawQ and QREADY and not myHero.dead then -- If the menu is ON and Q is ready and my hero is not dead then
		DrawCircle(myHero.x, myHero.y, myHero.z, Rrange, 0x00FF00) -- Draw a circle around me with Qrange(650) The color will be Green.
	end
end
...
function Combo()
	if ValidTarget(target) then -- Checking if our target is valid.
		if GetDistance(target) <= Wrange then
			SxOrb:DisableMove()
			SxOrb:EnableAttacks()
			if RREADY then CastSpell(_R) end -- Cast R
		else -- GAPCLOSING
		...
		if (getDmg("R", target, myHero) > target.health) and RREADY then
			SxOrb:DisableAttacks()
			CastSpell(_R, target.x, target.z) -- Cast R
		elseif WREADY and Config.Combo.UseW then -- Explained above.
			if GetDistance(target) <= Wrange then -- Explained above.
				CastSpell(_W) -- The W is targeted spell, so we will use it on our target
			end
		elseif EREADY and Config.Combo.UseE then -- Explained above.
				CastSpell(_E, target.x, target.z)
		elseif QREADY and Config.Combo.UseQ then -- Explained above.
				CastSpell(_Q, target.x, target.z)
		end
	end
end
